...
This folder contains the full empirical R pipeline used in the thesis on supplier performance and supplier segmentation in European public procurement.

The scripts are designed to be run in numerical order. Each stage reads the output of the previous stage, validates required inputs, and writes structured outputs to `data/processed`, `logs`, and `output`.
...
- **Local working format:** annual TED CAN CSV files stored locally and validated through the preparation pipeline.

Raw TED CAN files are not versioned in the repository because of their size. They are reconstructed locally by `01_download.R`, which downloads, extracts, and validates the required annual source files for 2018–2023.
...
- Validate parsed supplier-country codes against the ISO 3166-1 alpha-2 register; accept `UK` as a recognized special case for the United Kingdom in line with TED convention.
- Assign supplier-year `WIN_COUNTRY_CODE` as the modal non-missing parsed supplier-country code within each supplier-year.
- Record a `supplier_country_conflict` flag for supplier-years in which two or more distinct non-missing parsed country codes are observed across the awards in that year.
...
- Generate a validation log for observed `CRIT_CODE` values, including breakdowns by year and by procedure type, and a panel log for missingness and aggregation diagnostics.
...
The workflow is sequential, so each script depends on outputs generated by the previous one. If the raw TED CAN files are not already available locally, `01_download.R` prepares the raw-data inventory needed for the rest of the analysis.

## Relation to the thesis

This folder documents the empirical analysis only. The thesis manuscript, appendix drafting files, and other writing materials are maintained separately from the public replication repository.